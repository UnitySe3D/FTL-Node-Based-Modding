...
class FTLDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tokenized_text = self.tokenizer(xml_text, return_tensors="pt", padding=True, truncation=True)
...
def collate_tensor_fn(batch, device):
    ...
    input_ids = [entry["input_ids"][0].tolist() + [0] * (max_len - len(entry["input_ids"][0])) for entry in batch]
    attention_mask = [entry["attention_mask"][0].tolist() + [0] * (max_len - len(entry["attention_mask"][0])) for entry in batch]
...
def train_model(dataset, model: GPT2LMHeadModel, tokenizer, num_epochs=3, batch_size=16, learning_rate=2e-5, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")):
    ...
    with tqdm(desc='Creating data loader', total=100) as pbar:
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, collate_fn=lambda batch: collate_tensor_fn(batch, device))
        pbar.update(100)
    ...
    # Training loop
    with tqdm(iterable=num_epochs,desc='Training', total=num_epochs, unit="epochs") as pbar:
